...
class SBuild(implicit _project: Project) {
  ...
  Target("phony:all") dependsOn "compileScala-quick-compiler"
  ...
  def stagedJavac(stage: String, project: String, 
    ...
    Target("phony:" + stage) dependsOn t
    t
  }
  ...
  val lockerLibJavac = stagedJavac(
    stage = "locker", project = "library", 
    classpath = forkjoinJar,
    args = Seq("-XDignore.symbol.file")
  )

  val lockerLib = stagedScalac(
    stage = "locker", project = "library", compilerClasspath = starrCompilerCp,
    classpath = forkjoinJar ~ ant ~ lockerLibJavac,
    sourcepath = Some(Path("src/library"))
  ) ~ lockerLibJavac

  val lockerReflect = stagedScalac(
    stage = "locker", project = "reflect", compilerClasspath = starrCompilerCp,
    classpath = forkjoinJar ~ ant ~ lockerLib
  )

  val lockerCompiler = stagedScalac(
    stage = "locker", project = "compiler", compilerClasspath = starrCompilerCp,
  ...
  val quickLibJavac = stagedJavac(
    stage = "quick", project = "library", 
    classpath = forkjoinJar,
    args = Seq("-XDignore.symbol.file")
  )
  val quickLib = stagedScalac(
    stage = "quick", project = "library", compilerClasspath = lockerCompilerCp,
    classpath = forkjoinJar ~ ant ~ quickLibJavac,
    sourcepath = Some(Path("src/library"))
  ) ~ quickLibJavac

  val quickActorsJava = stagedJavac(
    stage = "quick", project = "actors", 
    // outputDir = Some("target/quick/library/classes"),
    classpath = forkjoinJar,
    args = Seq("-XDignore.symbol.file")
  )
  val quickActors = stagedScalac(
    stage = "quick", project = "actors",
    // outputDir = Some("target/quick/library/classes"),
    compilerClasspath = lockerCompilerCp,
    classpath = forkjoinJar ~ quickLib ~ quickActorsJava
  ) ~ quickActorsJava

  val quickReflect = stagedScalac(
    stage = "quick", project = "reflect", compilerClasspath = lockerCompilerCp,
    classpath = forkjoinJar ~ ant ~ quickLib
  )

  val quickCompiler = stagedScalac(
    stage = "quick", project = "compiler", compilerClasspath = lockerCompilerCp,
    classpath = forkjoinJar ~ asmClasses ~ ant ~ quickLib ~ quickReflect
  )

  val quickReplJava = stagedJavac(
    stage = "quick", project = "repl", 
    classpath = forkjoinJar ~ asmClasses ~ ant ~ quickLib ~ quickReflect ~ quickCompiler
  )
  val quickRepl = stagedScalac(
    stage = "quick", project = "repl", compilerClasspath = lockerCompilerCp,
    classpath = forkjoinJar ~ asmClasses ~ ant ~ jline ~ quickLib ~ quickReflect ~ quickCompiler ~ quickReplJava
  )

  val quickScalacheck = stagedScalac(
    stage = "quick", project = "scalacheck", compilerClasspath = lockerCompilerCp,
    classpath =  forkjoinJar ~ asmClasses ~ quickLib ~ quickReflect ~ quickCompiler ~ quickActors,
    args = Seq("-nowarn")
  )

  val quickScalap = stagedScalac(
    stage = "quick", project = "scalap", compilerClasspath = lockerCompilerCp,
    classpath =  forkjoinJar ~ asmClasses ~ quickLib ~ quickReflect ~ quickCompiler
  )


  // quick.scalacheck -> compiler* + lib-classes + scalacheck-classes
  // quick.scalap -> 
  // quick.interactive
  // quick.swing
  // quick.plugins
